...
private:
	...
	*/
	...
	/**
	Samples outgoing edges from `node` which is a single node. New activated nodes are added to
	...
	*/
	...
	{
		if (model_ == 0) 
		{ // Linear threshold model, this method isn't implemented for LT
		...
		{ // Independent Cascade model
			...
			{
				for (auto edge : graph.get_neighbours(node, inv))  // get_neighbours returns a vector type value
				{
					if (visited.find(edge.target) == visited.end()) // havent visited the target before
					{
						double dice_dst = edge.dist->sample(type_); // sample get probability
						unsigned int act = 0;
						double dice = dist_.gen_double();           // ramdom number generated by xorshift class
						if (dice < dice_dst) 
						{
						...
						if (trial) 
						{  // If trial, we want to save the generated RR set sample
							TrialType tt;
							tt.source = node;
